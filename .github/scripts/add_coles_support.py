...
'''
text = text.replace(marker, coles_script + marker, 1)

old = "const [preferences,setPreferences]=useState({matchMode:'best',preferSpecials:true,allowAlternatives:true,rememberBrands:true});"
new = "const [preferences,setPreferences]=useState({preferredSupermarket:'woolworths',matchMode:'best',preferSpecials:true,allowAlternatives:true,rememberBrands:true});"
if old not in text:
    raise SystemExit('preferences state missing')
text = text.replace(old, new, 1)
text = text.replace("  const [householdView,setHouseholdView]=useState('main');", "  const [colesResults,setColesResults]=useState(null);\n  const [householdView,setHouseholdView]=useState('main');", 1)
text = text.replace("setPreferences({matchMode:'best',preferSpecials:true,allowAlternatives:true,rememberBrands:true});", "setPreferences({preferredSupermarket:'woolworths',matchMode:'best',preferSpecials:true,allowAlternatives:true,rememberBrands:true});")
load_old = "        setPreferences({\n          matchMode:pref.match_mode==='cheapest'?'cheapest':'best',"
load_new = "        setPreferences({\n          preferredSupermarket:pref.preferred_supermarket==='coles'?'coles':'woolworths',\n          matchMode:pref.match_mode==='cheapest'?'cheapest':'best',"
if load_old not in text:
    raise SystemExit('preferences load block missing')
text = text.replace(load_old, load_new, 1)

anchor = "  function openWoolworthsFromAccount(){wooliesReturnTab.current='account';pending.current=[];injected.current=false;setStatus('Woolworths on this device');setCartUrl('https://www.woolworths.com.au/');setWebKey(k=>k+1);setMode('woolies')}\n"
if anchor not in text:
    raise SystemExit('Woolworths account anchor missing')
funcs = anchor + r'''  function compareColes(){if(!canSend)return;wooliesReturnTab.current='home';pending.current=items.map(i=>({name:i.name,qty:i.quantity,quantity:i.quantity,unit:i.unit}));injected.current=false;setColesResults(null);setStatus('Checking Coles…');setWebKey(k=>k+1);setMode('coles')}
...
woolies_render = "  if(mode==='woolies')return <SafeAreaView style={s.safe}><StatusBar barStyle=\"dark-content\"/><View style={s.cartHead}><TouchableOpacity onPress={back} style={s.back}><Text style={s.backText}>‹ {wooliesReturnTab.current==='account'?'Account':'Shop'}</Text></TouchableOpacity><View style={{flex:1}}><Text style={s.cartTitle}>Woolies</Text><Text style={s.cartStatus} numberOfLines={1}>{status}</Text></View></View><WebView key={webKey} ref={webRef} source={{uri:cartUrl}} style={{flex:1}} onMessage={onMessage} onLoadEnd={onLoad} userAgent={UA} javaScriptEnabled domStorageEnabled sharedCookiesEnabled thirdPartyCookiesEnabled cacheEnabled incognito={false} setSupportMultipleWindows={false}/></SafeAreaView>;\n"
if woolies_render not in text:
    raise SystemExit('Woolworths render missing')
coles_render = woolies_render + r'''
...
'''
text = text.replace(woolies_render, coles_render, 1)

pref_header = "      <Text style={s.sectionTitle}>Product matching</Text>\n"
if pref_header not in text:
    raise SystemExit('Preference header missing')
retailer_pref = r'''      <Text style={s.sectionTitle}>Preferred supermarket</Text>
...
'''
text = text.replace(pref_header, retailer_pref, 1)

connected_old = "        <MenuRow title=\"Woolworths\" sub=\"Sign in to Woolworths when you send your shopping\" right=\"Manage\" onPress={()=>setAccountView('woolworths')} />"
if connected_old not in text:
    raise SystemExit('Woolworths connected row missing')
text = text.replace(connected_old, connected_old + "\n        <MenuRow title=\"Coles\" sub=\"Coles login stays with Coles on this device\" right=\"Open\" onPress={openColesFromAccount} />", 1)

preferred_old = "        <MenuRow title=\"Preferred supermarket\" sub=\"Woolworths\" right=\"\" />"
if preferred_old not in text:
    raise SystemExit('Preferred supermarket row missing')
text = text.replace(preferred_old, "        <MenuRow title=\"Preferred supermarket\" sub={preferences.preferredSupermarket==='coles'?'Coles':'Woolworths'} onPress={()=>user?setAccountView('preferences'):setAccountView('auth')} />", 1)

home_anchor = "      <Text style={s.note}>We’ll build your Woolies cart. You review it before checkout.</Text>\n"
if home_anchor not in text:
    raise SystemExit('Home Woolies note missing')
home_coles = home_anchor + r'''      <TouchableOpacity style={[s.colesButton,!canSend&&s.sendOff]} onPress={compareColes} disabled={!canSend}>
