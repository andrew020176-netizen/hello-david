...
(async()=>{
 ...
 const baseQuery=i=>String(i.name||'').replace(/\b(cheapest suitable|on special|cheapest|best value|value option)\b/gi,'').replace(/\s+/g,' ').trim().replace(/weet\s*-?\s*a?bix|weetbix|weet bix/gi,'Weet-Bix');
 ...
 try{post('WOOLIES_STATUS','Matching your groceries…');const chosen=[],unmatched=[],used=new Set();for(let n=0;n<items.length;n++){const i=items[n];post('WOOLIES_STATUS','Matching '+(n+1)+' of '+items.length+': '+i.name);let ps=[];try{ps=await search(i)}catch(_){unmatched.push(i.name);continue}const ws=words(query(i)),needed=ws.length===1?1:Math.min(2,ws.length);let ranked=ps.map(p=>({p,s:score(i,p),hits:ws.filter(w=>txt(p).includes(w)).length,price:+(p.Price||999)})).filter(x=>x.p?.Stockcode&&x.s>=28&&x.hits>=needed);if(!ranked.length){unmatched.push(i.name);continue}const bestScore=Math.max(...ranked.map(x=>x.s));ranked=ranked.filter(x=>x.s>=bestScore-(prefs.allowAlternatives?35:12));if(!prefs.allowAlternatives){const exactQuery=query(i).toLowerCase();ranked=ranked.filter(x=>x.hits===ws.length||txt(x.p).includes(exactQuery));if(!ranked.length){unmatched.push(i.name);continue}}ranked.sort((a,b)=>(prefs.matchMode==='cheapest'||/\b(cheapest|best value|value option)\b/.test(req(i)))?(a.price-b.price||b.s-a.s):(b.s-a.s||a.price-b.price));const p=ranked[0]?.p;if(!p||used.has(String(p.Stockcode))){unmatched.push(i.name);continue}used.add(String(p.Stockcode));chosen.push([i,p]);await sleep(120)}let added=0,failed=0,failedNames=[],remembered=[];for(let n=0;n<chosen.length;n++){const i=chosen[n][0],p=chosen[n][1];post('WOOLIES_STATUS','Adding '+(n+1)+' of '+chosen.length+': '+i.name);if(await add(i,p)){added++;remembered.push({request:i.name,productId:String(p.Stockcode),productName:p.DisplayName||p.Name||'',brand:p.Brand||'',size:p.PackageSize||''})}else{failed++;failedNames.push(i.name)}await sleep(220)}const message=added+' products added.'+(unmatched.length?' Could not confidently match: '+unmatched.join(', ')+'.':'')+(failed?' Failed to add: '+failedNames.join(', ')+'.':'');post('WOOLIES_DONE',message,{added,failed,failedNames,unmatched,remembered,success:added>0})}catch(e){post('WOOLIES_DONE','Could not finish the Woolies shop.',{added:0,success:false})}
 return true;
})();true;`;
}
